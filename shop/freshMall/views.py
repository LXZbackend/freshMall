...
def detail(request, dic):
    ...
    recommendGoods = GoodsList.objects.filter(
        goodsType=goodclass).order_by('-goodsPubdate')[0:4]
...
def addCart(request, dic):
    ...
    # 后面加上filter(isSettle=False)  判断这个是否已经被结算了 如果被结算了就从新创建一个新的,如果每又被结算,就直接把数量和小计加上.
    flag = ShoppingCart.objects.filter(
        userId=per.id).filter(goodsId=good.id).filter(isSettle=False)
...
def list(request, dic):
    ...
    # 推荐商品(当前类对应下的列表)
    recommendGoods = goodclass.goodslist_set.all(
    ).order_by('-goodsPubdate')[0:3]
    # 分页
